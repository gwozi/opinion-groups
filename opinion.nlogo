...
to setup
  ...
  create-turtles agent-num [
    ...
    set true-opinion random-float 100
    set next-opinion true-opinion
...
to gather-opinion
  let nearby turtles in-radius 2
  set near-opinion ifelse-value any? nearby [mean [expr-opinion] of nearby] [50]
end
...
to express-opinion
  set expr-opinion next-opinion
  let hue ifelse-value expr-opinion < 50 [250] [0]
  let saturation 2 * abs(expr-opinion - 50)
  set color hsb hue saturation 100
end
...
to update-plot
  ...
  plot-pen-down
  plotxy ticks expr-opinion
  plot-pen-up
end

to-report bound [ value low high ]
  report ifelse-value value < low [low] [ ifelse-value value > high [high] [value] ]
end
@#$#@#$#@
GRAPHICS-WINDOW
476
